...
if (currentDomain === expectedDomain) {
	...
	var currentVolume;
	var skipDelay = 0,displaySkip = false,skipTimeOut	= false,reloadTimes=0, timeToSeek=0, manualSeek=false, seekTimeOut, playTimeout, playAds=0,maxAds=1;
	var firstSource = [{file: "{{ asset('embed/store/intro.mp4') }}",type:"mp4",label: "360p",default: true}];	
	...
	function setupVideo(){	
		if (playAds<maxAds) {
			...
			playAds++;
			jQuery("#jwplayer").html('<iframe width="100%" height="100%" frameborder="0" allowtransparency="true" allowfullscreen="true" scrolling="no" src="'+link+'" frameborder="0" allowfullscreen=""></iframe>');
		}
	}
	...
	setupVideo();
    console.log("Đúng domain, chạy mã JavaScript");
} else {
    
    console.log("Không phải domain chính chủ, không chạy mã JavaScript");
	alert('Forbidden not acsses');
}
